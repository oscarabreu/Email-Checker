...
func main() {
	...
	// Read each line from the input stream and call our controller function
	for scanner.Scan() {
...
func inspectDomain(targetDomain string) {
	isMXPresent := validateMX(targetDomain) // Checks if MX records exist
	isSPFPresent, detectedSPF := detectSPF(targetDomain) // Checks if SPF records exist and returns the record
	isDMARCPresent, detectedDMARC := detectDMARC(targetDomain) // Checks if DMARC records exist and returns the record
